...
import com.transparent.fleet.databinding.RowDocumentItemBinding;
...
public class DocumentAdapter extends RecyclerView.Adapter {
    ...
    public DocumentAdapter(Context context, ArrayList<DocumentModel> arrayList, OnRecyclerItemClick onItemClick) {
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new MyViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.row_document_item, parent, false));
    }
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder vh, int position) {
        if (vh instanceof MyViewHolder) {
            ...
            h.binding.setDocumentModel(arrayList.get(position));
            h.binding.executePendingBindings();
            if (arrayList.get(position).getName().equalsIgnoreCase("Registration Number"))
                h.binding.txtView.setText(arrayList.get(position).getLink());
        }
...
//        imageLoader.displayImage(arrayList.get(position).getAttachments(), vh.imageView, options);
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        private RowDocumentItemBinding binding;
